...
public class EmployeeServlet extends CommonServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (obj != null) {
			...
			out.println(
					"<div class=\"error\">You have not logged in properly. Please login <a href=\"/EMP\">here</a></div>");
		}
	...
	private void delete(HttpServletRequest request, HttpServletResponse response, PrintWriter out)
		...
		if ("delete".equals(opr)) {
			...
			try {
				...
				PreparedStatement stmt = commonDAO.prepare("DELETE FROM EMPLOYEES WHERE emp_no = ?");
	...
	private void maintain(String opr, HttpServletRequest request, PrintWriter out) {
		...
		/**
		 * emp_no int(11) PK birth_date date first_name varchar(14) last_name
		 * varchar(16) gender enum('M','F') hire_date date
		 */
		...
		if (isEdit) {
			...
			try {
				...
				ResultSet result = commonDAO.getRecord("EMPLOYEES", "emp_no", request.getParameter("emp_no"));
		...
		out.println(
				"       <input name=\"EmployeeNumber\" type=\"text\" id=\"EmployeeNumber\" class=\"form-control\" value=\""
						+ id + "\" " + readonly + " />");
		...
		out.println("       <input name=\"FirstName\" type=\"text\" id=\"FirstName\" class=\"form-control\" value=\""
				+ fname + "\" />");
		out.println("     </div>");
		...
		out.println("       <input name=\"LastName\" type=\"text\" id=\"LastName\" class=\"form-control\" value=\""
				+ lname + "\" />");
		out.println("       </div>");
		out.println("       </div>");
		...
		out.println("       <input name=\"HireDate\" type=\"text\" id=\"HireDate\" class=\"form-control\" value=\""
				+ hdate + "\" />");
		out.println("     </div>");
		...
		out.println(
				"       <button id=\"employeeSubmit\" name=\"employeeSubmit\" type=\"submit\" class=\"btn btn-primary\">Submit</button>");
		out.println("     </div>");
	...
	private void view(HttpServletRequest request, PrintWriter out) {
		...
		try {
			...
			// Execute the Select Query
			ResultSet result = commonDAO.getTableData("employees", "last_name");
			...
			while (result.next()) {
				...
				out.println("      <tr>");
				out.println("        <td headers=\"maintain\"><a href=\"/EMP/employees?opr=edit&emp_no=" + id
						+ "\">Edit</a></td>");
				...
				out.println("        <td headers=\"delete\"><a href=\"/EMP/employees?opr=delete&emp_no=" + id
						+ "\">Delete</a></td>");
				out.println("      </tr>");
				count++;
			}
			...
			out.println("    <tfoot>");
			out.println("      <tr>");
			out.println("        <td class=\"text-center\" colspan=\"8\"><b>Total: " + count + "</b></td>");
			out.println("      </tr>");
			out.println("    </tfoot>");
			out.println("</table>");
			out.println("</figure>");
		...
		if (obj != null) {
			...
			if ("addSubmit".equals(opr)) {
				try {
					...
					// Create a database statement
					PreparedStatement stmt = commonDAO.prepare(
